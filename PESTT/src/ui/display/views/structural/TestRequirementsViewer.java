...
public class TestRequirementsViewer extends AbstractTableViewer implements
		Observer {
	...
	public TableViewer create() {
		testRequirementsViewer = createViewTable(parent, site,
				TableViewers.TESTREQUIREMENTSVIEWER);
	...
	private void createColumnsToTestRequirement() {
		...
		TableViewerColumn col = createColumnsHeaders(testRequirementsViewer,
		...
		col = createColumnsHeaders(testRequirementsViewer, columnNames[1],
				columnWidths[1], 1);
		col.setLabelProvider(new StyledCellLabelProvider() {
		...
		col = createColumnsHeaders(testRequirementsViewer, columnNames[2],
				columnWidths[2], 2);
		col.setLabelProvider(new StyledCellLabelProvider() {
	...
	private void cleanPathStatus() {
		...
		for (TableItem item : testRequirementsViewer.getTable().getItems()) {
			...
			testRequirementsViewer.getTable().getItem(n)
	...
	private void setPathStatus() {
		...
		for (TableItem item : testRequirementsViewer.getTable().getItems()) {
			...
			} else if (item.getText(2).equals(path.toString())
					&& item.getChecked()) {
				item.setText(0, Integer.toString(n + 1));
				item.setImage(1, images.getImage().get(Images.PASS));
	...
	/***
	 * Show all test requirements associated to the selected def-use (variable
						...
						if (event.detail == SWT.CHECK) { // when user enable/disable an infeasible path.
							...
							for (TableItem item : testRequirementsViewer
								...
								if (item == event.item) {
									Activator.getDefault()
											.getTestRequirementController()
											.selectTestRequirement(selected);
									break;
								}
							}
						}
