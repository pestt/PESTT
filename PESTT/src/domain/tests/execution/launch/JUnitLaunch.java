...
public class JUnitLaunch extends JUnitLaunchShortcut {
	...
	private static final String BYTEMAN_JAR = "lib" + IPath.SEPARATOR
			+ "byteman.jar";
	...
	public void launch(IEditorPart editor, String mode) {
		...
		Activator.getDefault().getTestPathController()
				.clearAutomaticTestPaths();
		super.launch(editor, mode);
		IEditorPart part = Activator.getDefault().getEditorController()
				.getEditorPart();
		...
		try {
			...
			if (configurations.length != 0) {
				...
				IRuntimeClasspathEntry projectEntry = JavaRuntime
						.newDefaultProjectClasspathEntry(Activator.getDefault()
								.getEditorController().getJavaProject());
				projectEntry
						.setClasspathProperty(IRuntimeClasspathEntry.USER_CLASSES);
				// Add plugin folder - for finding the byteman helper classes
				String pluginPath = new File(FileLocator.toFileURL(
						Platform.getBundle(Activator.PLUGIN_ID).getEntry("/"))
						.toURI()).getAbsolutePath();
				IRuntimeClasspathEntry pluginEntry = JavaRuntime
				...
				workingCopy.setAttribute(
						...
						"-javaagent:" + bytemanPath + "=script:"
								+ Byteman.SCRIPT_DIR + IPath.SEPARATOR
								+ Byteman.SCRIPT_FILE);
			...
			MessageDialog.openInformation(window.getShell(),
					Messages.PREFERENCES_TITLE, Messages.PREFERENCES_TOOLS_MSG);
			return;
